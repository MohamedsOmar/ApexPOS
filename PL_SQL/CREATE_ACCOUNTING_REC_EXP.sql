...
BEGIN
    ...
    -- Get Records Created
    SELECT COUNT(*)
    INTO l_last_gl_created
    FROM APP_EXPENSES
    WHERE TRUNC(LAST_GL_CREATED) BETWEEN TRUNC(SYSDATE, 'MONTH')  AND LAST_DAY(TRUNC(SYSDATE));
    ...
    SELECT 
        1,EXPENSE_ACC, TRUNC(sysdate), EXPENSE_AMT, 'Reccurring Expenses, '|| TRUNC(sysdate) || ' - ' || l_create_accounting_comment || ' ('|| ENTRY_DESCRIPTION || ')',
        'Reccurring Exopenses Accounting Porcess',l_user_id, l_user_id, SYSDATE, COMPANY_ID
    FROM (
        SELECT
            EXPENSE_ACC,
            EXPENSE_AMT,
            ...
            COMPANY_ID
        FROM APP_EXPENSES
        WHERE TRUNC(LAST_GL_CREATED) NOT BETWEEN TRUNC(SYSDATE, 'MONTH')  AND LAST_DAY(TRUNC(SYSDATE))
            OR LAST_GL_CREATED IS NULL
    );
    ...
    SELECT 
        1,CASH_ACC, TRUNC(sysdate), EXPENSE_AMT, 'Reccurring Expenses, '|| TRUNC(sysdate) || ' - ' || l_create_accounting_comment || ' ('|| ENTRY_DESCRIPTION || ')',
        'Reccurring Exopenses Accounting Porcess',l_user_id, l_user_id, SYSDATE, COMPANY_ID
    FROM (
        SELECT
            CASH_ACC,
    ...
    BEGIN
        UPDATE APP_EXPENSES SET  LAST_GL_CREATED = SYSDATE;
    END;
