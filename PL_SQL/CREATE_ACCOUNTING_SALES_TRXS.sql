DECLARE
    l_defatul_inventory_ACC_ID          NUMBER;
    l_sales_revenue_ACC_ID          NUMBER;
    l_sales_discount_ACC_ID          NUMBER;
...
BEGIN
    ...
    BEGIN
        SELECT ACC_ID
        INTO l_defatul_inventory_ACC_ID
        FROM APP_ACCOUNTS_MAPPING
        WHERE ACC_INT_NAME_ID = 'INVENTORY_DEFAULT_ACCOUNT'
        AND ROWNUM = 1;
        ...
        SELECT ACC_ID
        INTO l_sales_revenue_ACC_ID
        FROM APP_ACCOUNTS_MAPPING
        WHERE ACC_INT_NAME_ID = 'SALES_REVENUE_ACCOUNT'
        AND ROWNUM = 1;

        SELECT ACC_ID
        INTO l_sales_discount_ACC_ID
        FROM APP_ACCOUNTS_MAPPING
        WHERE ACC_INT_NAME_ID = 'SALES_DISCOUNT_ACCOUNT'
        AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            l_defatul_inventory_ACC_ID:= null;
            l_sales_discount_ACC_ID:= null;
            l_sales_revenue_ACC_ID:= null;
            l_cogs_ACC_ID:= null;
    END;

    IF l_defatul_inventory_ACC_ID IS NULL OR l_sales_revenue_ACC_ID IS NULL OR l_sales_discount_ACC_ID IS NULL OR l_cogs_ACC_ID IS NULL THEN
        apex_json.open_object;
            apex_json.write('status', 'Error');
            apex_json.write('rowsProccessed', 0);
            apex_json.write('message','Some of Default Accounts is not Defined');
    ...
    -- Bulk INSERT Sales Cash In with Payment Methods
    ...
    SELECT 
        1,C_CHILD_ACCOUNT, TRUNC(PAYMENT_DATE), PAYMENT_AMT, 'POS Payments, '|| TRUNC(PAYMENT_DATE) || ' - ' || l_create_accounting_comment,'POS Page',l_user_id, l_user_id, SYSDATE, INV_BRANCH
    FROM (
        SELECT 
            TRUNC(sip.PAYMENT_DATE) AS PAYMENT_DATE,
            SUM(sip.PAYMENT_AMT) AS PAYMENT_AMT,
            pm.C_CHILD_ACCOUNT,
            sih.INV_BRANCH
        FROM SALES_INV_PAY sip,
            SALES_INV_HEADER sih,
            PAYMETN_METHODS pm
        WHERE 1=1 
            and pm.PAY_METHOD_ID = sip.PAY_METHOD
            and sip.INV_ID = sih.INV_ID
            and sip.POSTED_REC <> 1
            AND sip.DELETED <> 1
            AND sih.INV_CLOSED  <> 0
        GROUP BY TRUNC(sip.PAYMENT_DATE), pm.C_CHILD_ACCOUNT, sih.INV_BRANCH
        HAVING SUM(sip.PAYMENT_AMT) <> 0
    );
    l_rows_affected := SQL%ROWCOUNT + l_rows_affected;
    -- Bulk INSERT Sales Revnue
    ...
    SELECT 
        1,l_sales_revenue_ACC_ID, TRUNC(PAYMENT_DATE), PAYMENT_AMT, 'POS Payments, '|| TRUNC(PAYMENT_DATE) || ' - ' || l_create_accounting_comment,'POS Page',l_user_id, l_user_id, SYSDATE, INV_BRANCH
    FROM (
        SELECT 
            TRUNC(sip.PAYMENT_DATE) AS PAYMENT_DATE,
            SUM(sip.PAYMENT_AMT) AS PAYMENT_AMT,
        ...
        WHERE 1=1 
            ...
            AND sip.DELETED <> 1
            AND sih.INV_CLOSED  <> 0
        GROUP BY TRUNC(sip.PAYMENT_DATE), sih.INV_BRANCH
        HAVING SUM(sip.PAYMENT_AMT) <> 0
    );
    l_rows_affected := SQL%ROWCOUNT + l_rows_affected;
    -- Bulk INSERT Sales Discount
    INSERT INTO APP_Ledger(POSTED_REC,ENTRY_ACC,TRX_DATE,DEBIT_SIDE,ENTRY_DESCRIPTION,ENTRY_SRC,CREATED_BY,UPDATED_BY,POST_DATE,COMPANY_ID)
    SELECT 
        1,l_sales_discount_ACC_ID, TRUNC(INV_DATE), total_discount, 'POS Payments, '|| TRUNC(INV_DATE) || ' - ' || l_create_accounting_comment,'POS Page',l_user_id, l_user_id, SYSDATE, INV_BRANCH
    FROM (
        SELECT 
            TRUNC(sih.INV_DATE) INV_DATE,
            SUM(sil.LINE_DISCOUNT)total_line_discounts,
            SUM(DISTINCT sih.DISCOUNT)total_header_discount,    
            sih.INV_BRANCH ,
            SUM(sil.LINE_DISCOUNT) + SUM(DISTINCT sih.DISCOUNT)   AS total_discount
        ...
        GROUP BY TRUNC(sih.INV_DATE), sih.INV_BRANCH
        HAVING SUM(sil.LINE_DISCOUNT) + SUM(DISTINCT sih.DISCOUNT) <> 0
    );
    l_rows_affected := SQL%ROWCOUNT + l_rows_affected;

    -- Updating Post Status for Records in sales Invoice Paymnets
    BEGIN
        UPDATE sales_inv_pay sip
        SET    posted_rec = 1
        WHERE  sip.posted_rec <> 1
        AND  sip.deleted    <> 1
        AND  sip.payment_amt <> 0
        AND  EXISTS (
            SELECT 1
            FROM   sales_inv_header sih
            WHERE  sih.inv_id     = sip.inv_id
            AND  sih.inv_closed <> 0
        );
