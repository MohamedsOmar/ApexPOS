        ...
        FOR r IN (
            SELECT
                ...
                LOW_STOCK,
                BALANCE
            FROM SI_LINES_DETAILS
            WHERE 1=1
                and INV_ID = l_invoiceID
                AND line_deleted <> 1
            ORDER BY LINE_CREATION_DATE
        ) LOOP
            apex_json.open_object;           
            apex_json.write('invID',           r.INV_ID);
            apex_json.write('invLineID',       r.INV_LINE_ID);
            apex_json.write('itemID',          r.ITEM_ID);
            apex_json.write('itemName',        r.ITEM_NAME);
            apex_json.write('uom',             r.UOM);
            apex_json.write('qty',             r.QTY);
            apex_json.write('taxAmount',       r.TAX_AMT);
            apex_json.write('taxRateValue',    r.TAX_RATE);
            apex_json.write('taxRateID',       r.TAX_RATE_ID);
            apex_json.write('lineAfterTax',    r.TOTAL_AFTER_TAX);
            apex_json.write('lineOfferAmt',    r.LINE_OFFER);
            apex_json.write('lineDiscount',    r.LINE_DISCOUNT);
            apex_json.write('categoryID',      r.CAT_ID);
            apex_json.write('ItemBasePrice',   r.ITEM_BASE_PRICE);
            apex_json.write('lowStockCheck',   r.LOW_STOCK);
            apex_json.write('itemBalance',     r.BALANCE);
            apex_json.write('totalBeforeTax',  r.TOTAL_BEFORE_TAX);
    ...
    if (data.found === 'Y') {
        ...
        jsonData.forEach((item)=>{
            ...
            div.innerHTML = `
                ...
                <div class="itemTaxRateID"  style="display:none">${item.itemTaxRateID}</div>
                <div class="itemTaxAmount"  style="display:none">${item.itemTaxAmount}</div>
                <div class="itemBasePrice"  style="display:none">${item.itemBasePrice}</div>
                <div class="itemTaxRateValue"  style="display:none">${item.itemTaxRateValue}</div>
                <div class="item-wrapper w-100 h-100">
                    ...
                    <div class="d-flex-r cntnt-sb w-100" style="padding: 5px 10px;" >
                        <div class="item_price">${formatAccounting(item.item_price)}
                            <span>${branchCurrencySymbole}</span>
                        </div>
...
try{
    ...
    try {
        ...
        soldItems.forEach(row => {
            ...
            const itemTaxRateValue    = row.querySelector(".itemTaxRateValue")?.textContent.trim();
            const price = parseFloat(priceEl?.textContent?.replace(/,/g, '')) || 0;
            const qty   = parseInt(qtyInput?.value || '0', 10) || 0;
            console.log(`priceEl: ${price} qtyInput:${qty}`)
            if (qty <= 0) return; 
            const amount = qty * price;
            saveLine(row, qty, price,amount, true, itemTaxRateID, itemTaxAmount, itemBasePrice, itemTaxRateValue);
        });
        triggerChangedInvoice()
        fetchOpenInvoices();
...
let itemID, itemsRowCount,itemName,itemPrice,itemUOM,itemTaxRateID,itemTaxAmount,itemTaxRateValue,itemBasePrice,itemImg
...
async function handleItemClick(clickedRow) {
    ...
    try {
        ...
        itemID   = row.querySelector(".item_id").textContent.trim();
        if (!itemID) return;
        itemName = row.querySelector(".item_name").textContent.trim();
        itemPrice = parseFloat(row.querySelector(".item_price").textContent.replace(/,/g, '')) || 0;
        itemTaxRateID = row.querySelector(".itemTaxRateID").textContent;
        itemTaxAmount = parseFloat(row.querySelector(".itemTaxAmount").textContent.replace(/,/g, '')) || 0;
        itemBasePrice = parseFloat(row.querySelector(".itemBasePrice").textContent.replace(/,/g, '')) || 0;
        itemTaxRateValue = parseFloat(row.querySelector(".itemTaxRateValue").textContent.replace(/,/g, '')) || 0;
        itemUOM  = row.querySelector(".item_uom").textContent.trim();
        itemImg = row.querySelector('img').getAttribute("src")?.trim() || "";
        ...
        let newRowHtml = createNewPOSrow(itemID, itemsRowCount,itemName,itemPrice,itemUOM,itemTaxRateID,itemTaxAmount,itemTaxRateValue,itemBasePrice,itemImg)
    ...
    function posTableLines(trSelected, currentQty) {
        ...
        const amount    = qty * itemPrice;
        row.querySelectorAll(".amount").forEach(ele=>ele.textContent=formatAccounting(amount.toFixed(2)));
        if (invoiceID) {
            if(invLineID){
                updateLine(itemID, qty, itemPrice, amount, invLineID, itemTaxRateID, itemTaxAmount, itemBasePrice, itemTaxRateValue);
            }else {
                saveLine(row, 1, itemPrice, itemPrice, false, itemTaxRateID, itemTaxAmount, itemBasePrice, itemTaxRateValue);
            }
        }
        if(!invoiceID){
            if(!itemFound){
...
function createNewPOSrow(itemID, itemsRowCount,itemName,itemPrice,itemUOM,itemTaxRateID,itemTaxAmount,itemTaxRateValue,itemBasePrice,itemImg){
    ...
    newRowHtml.innerHTML = `
        ...
        <div style="display: none;" class="itemTaxRateValue">${itemTaxRateValue}</div>
...
/* ------------------------ Save Line to DB ------------------------ */
async function saveLine(row, qty, price, amount, newInv, itemTaxRateID, itemTaxAmount, itemBasePrice,itemTaxRateValue) {
try{
    if(!openShiftID){openNewShift(); return}
    ...
    let data = await apex.server.process("ADD_INV_LINE_TO_DB",{x01: itemID,x02: qty,x03: price,x04: amount,x05: invoiceID,x06: lineType,
        x07:itemTaxRateID, x08:itemTaxAmount, x09:itemBasePrice, x10: itemTaxRateValue
    },{dataType: "json"})
    if (data.status !== "SUCCESS") {
...
/* ------------------------ Update DB Line ------------------------- */
async function updateLine(itemID, qty, price, amount, invLineID, itemTaxRateID, itemTaxAmount, itemBasePrice, itemTaxRateValue) {
try{
    if(!openShiftID){openNewShift(); return}
